...
dataEffort <- subset.data.frame(
  dataArea, subset = c(variable_name == "kWfishing days"), select = c("country_name", "year","variable_name","value")
)
...
p.dataCountry <- ddply(dataEffort, .(country_name, year), 
                     summarize,
                     "kw_fishingdays" = sum(value))

#- convert to wide format, and sort individually small => large per column.
dataCountry <- dcast(p.dataCountry, year ~ country_name, value.var = "kw_fishingdays", fun.aggregate = sum)
